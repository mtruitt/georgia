...
class FrmProFieldsHelper {
	...
	*/
	...
	private static function maybe_remove_separate_value( $field, &$opt ) {
		...
		if ( FrmField::is_field_type( $field, 'product' ) || ! is_array( $opt ) || ! isset( $opt['label'] ) ) {
			return;
		}
...
<?php
	...
	*/
	public static function insert_extra_hidden_fields( $field, $opt_key = false ) {
		...
		if ( isset( $field['original_type'] ) && ( $field['original_type'] === 'product' || $field['original_type'] === 'total' ) ) {
