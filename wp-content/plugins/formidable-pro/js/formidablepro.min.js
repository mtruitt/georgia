...
onCurrentPage){if(isFieldCurrentlyShown(depFieldArgs.containerId,depFieldArgs.formId))return;removeFromHideFields(depFieldArgs.containerId,depFieldArgs.formId);if(depFieldArgs.fieldType==="submit"){if(onCurrentPage)showOrEnableSubmitButton(depFieldArgs);return}if(onCurrentPage){setValuesInsideFieldOnPage(depFieldArgs.containerId,depFieldArgs);showFieldContainer(depFieldArgs.containerId)}else setValuesInsideFieldAcrossPage(depFieldArgs)}function showOrEnableSubmitButton(depFieldArgs){if(depFieldArgs.hideDisable&&
...
required){if(inputs.length<1)return;var prevInput,blankSelect,valueChanged=true;for(var i=0,l=inputs.length;i<l;i++){if(inputs[i].className.indexOf("frm_dnc")>-1||inputs[i].name.indexOf("[row_ids]")>-1){prevInput=inputs[i];continue}if(i>0&&prevInput.name!=inputs[i].name&&valueChanged===true)triggerChange(jQuery(prevInput));valueChanged=true;if(inputs[i].type==="radio"||inputs[i].type==="checkbox")inputs[i].checked=false;else if(inputs[i].tagName==="SELECT"){blankSelect=inputs[i].selectedIndex===0&&
inputs[i].options[0].text.trim()==="";if(blankSelect||inputs[i].selectedIndex===-1)valueChanged=false;else inputs[i].selectedIndex=-1;var chosenId=inputs[i].id.replace(/[^\w]/g,"_");var autocomplete=document.getElementById(chosenId+"_chosen");if(autocomplete!==null)jQuery(inputs[i]).trigger("chosen:updated")}else if(inputs[i].type==="range")inputs[i].value=0;else if(inputs[i].getAttribute("data-frmprice")!==null)setHiddenProduct(inputs[i]);else inputs[i].value="";if(required==="required"){inputs[i].required=
...
"").split("-")[0];checked.push(fieldID);hideOrShowFieldById(fieldID,repeatArgs)});loadDropzones(repeatArgs.repeatRow);loadSliders();loadChosen()}if(typeof frmThemeOverride_frmAddRow=="function")frmThemeOverride_frmAddRow(id,r);jQuery(document).trigger("frmAfterAddRow");jQuery(".frm_repeat_"+id).each(function(i){this.style.zIndex=999-i});currentlyAddingRow=false},error:function(){currentlyAddingRow=false}});return false}function editEntry(){var $edit=jQuery(this),entryId=$edit.data("entryid"),prefix=
$edit.data("prefix"),postId=$edit.data("pageid"),formId=$edit.data("formid"),cancel=$edit.data("cancel"),fields=$edit.data("fields"),excludeFields=$edit.data("excludefields"),$cont=jQuery(document.getElementById(prefix+entryId)),orig=$cont.html();$cont.html('<span class="frm-loading-img" id="'+prefix+entryId+'"></span><div class="frm_orig_content" style="display:none">'+orig+"</div>");jQuery.ajax({type:"POST",url:frm_js.ajax_url,dataType:"html",data:{action:"frm_entries_edit_entry_ajax",post_id:postId,
entry_id:entryId,id:formId,nonce:frm_js.nonce,fields:fields,exclude_fields:excludeFields},success:function(html){$cont.children(".frm-loading-img").replaceWith(html);$edit.removeClass("frm_inplace_edit").addClass("frm_cancel_edit");$edit.html(cancel);checkConditionalLogic("editInPlace");if(typeof frmFrontForm.fieldValueChanged==="function")jQuery(document).on("change",'.frm-show-form input[name^="item_meta"], .frm-show-form select[name^="item_meta"], .frm-show-form textarea[name^="item_meta"]',frmFrontForm.fieldValueChanged);
...
el.value.trim())}totalFields=jQuery("[data-frmtotal]");if(!totalFields.length)return;totalFields.each(function(){var currency,formId,total=0,totalField=jQuery(this),$form=totalField.closest("form"),isRepeatingTotal=isRepeatingFieldByName(this.name);if(!$form.length)return;formId=$form.find('input[name="form_id"]').val();currency=getCurrency(formId);if(typeof formTotals[formId]!=="undefined"&&!isRepeatingTotal)total=formTotals[formId];else{$form.find("input[data-frmprice],select:has([data-frmprice])").each(function(){var quantity,
